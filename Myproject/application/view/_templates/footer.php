...
<script src="<?php echo URL; ?>/bootstrap/js/bootbox.min.js"></script>
<!-- jQuery UI 1.11.4 -->
...
<script src="<?php echo URL; ?>/plugins/fullcalendar/fullcalendar.min.js"></script>
<!-- DataTables -->
...
<!-- Patients page scripts -->
<script>
    
    $(function (){
         $.ajax({
            method: "POST",
            url: "/patients/getpatientslist",
            data: { limit: 10 }
        }).done(function( msg ) {
            $('#patientsTable tbody').html(msg);
        });
    });
  ...
  $(function () {
    ...
    function hidePage(page){
        page.addClass('content-hide');
        page.css('right','-1500px');
    }
    
    function loadPatient(position,patientID,elm,pageToShow){
        var patientPage;
        $.ajax({
            method: "POST",
        ...
        elm.one('webkitAnimationEnd oanimationend msAnimationEnd animationend', function(e) {
            position.html(patientPage);
        });
    }
    
    $(document).on('click', '#patientsTable tbody tr', function(){
        var patientID = $(this).attr("patient-id");
        var elm = $('.patientPage');
        var parent = elm.parent();
        hidePage(elm);
        loadPatient(parent,patientID,elm);
    });
